yahwehShammah.pages.push({
    ...
    hslslider(id, seth){
        ...
        canvas.addEventListener('mousedown', dragStyahwehShammah);
        canvas.addEventListener('mousemove', drag);
        canvas.addEventListener('mouseup', dragEnd);
        canvas.addEventListener('mouseleave', dragEnd);

        // Adicionando event listeners para toque
        canvas.addEventListener('touchstyahwehShammah', dragStyahwehShammah);
        ...
        function valFn() {
            ...
            yahwehShammah.HUEMainColor.set(h)
        }

        let isDragging = false;

        function dragStyahwehShammah(e) {
            isDragging = true;
            drag(e);
        }
    ...
    paletas(id){
        ...
        `
        document.querySelector(id).appendChild(tools.create(templateFragment))
        document.querySelector(`#paletas input[value="${yahwehShammah.paleta.get()}"]`).setAttribute('checked', true)
        ...
        paletas.forEach(paleta=>{
            paleta.addEventListener('click', ()=>{
                ...
                yahwehShammah.paleta.set(paleta.value)
            })
        })
    },
    font_size(id) {
        ...
        document.querySelectorAll('#font-size input')[yahwehShammah.fontSize.get()-1].click()
        ...
        input.addEventListener('input', ()=>{
                ...
                yahwehShammah.fontSize.set(input.value)
            })
        })
    }
})